...
public protocol PinTextFieldDelegate: AnyObject {
    func pinTextField(_ field: PinTextField, didChangeTo string: String, isValid: Bool) // Optional
...
public struct PinTextFieldProperties {
...
public struct PinTextFieldAppearance {
...
public class PinTextField: UITextField {
    ...
    public var properties = PinTextFieldProperties() {
        didSet {
            self.reload()
        }
    }

    public var appearance = PinTextFieldAppearance() {
        didSet {
    ...
    @objc func doneWithNumberPad() {
        self.properties.delegate?.pinTextField(self, didFinishWith: self.invisibleText)
    }
    ...
    private func sanitizeText() {
        ...
        if text != lastEntry {
            ...
            if text.count <= self.properties.numberOfCharacters {
                self.properties.delegate?.pinTextField(self, didChangeTo: text, isValid: isValid)
            }
    ...
    private func checkCodeValidity() {
        ...
        if self.invisibleText.count == self.properties.numberOfCharacters {
            if let pinDelegate = self.properties.delegate {
                ...
                pinDelegate.pinTextField(self, didFinishWith: result)
            } else {
                print("⚠️ : No delegate set for PinTextField. Set it via yourPinField.properties.delegate.")
            }
        }
    }
}
